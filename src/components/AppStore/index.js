...
class AppStore extends Component {
  ...
  getSearchResults = active => {
    const {searchInput} = this.state
    const searchResults = active.filter(each =>
  ...
  render() {
    ...
    const activeTabResults = this.getFilteredApps()
    const searchResults = this.getSearchResults(activeTabResults)
            ...
            <ul className="tabs-main">
              {tabsList.map(each => (
                <TabItem
                  key={each.tabId}
                  details={each}
                  clickItem={this.clickedTab}
                  active={activeTab === each.tabId}
                />
              ))}
            </ul>
          </div>
          <div className="app-container">
            ...
            <ul className="app-main">
              {searchResults.map(each => (
                <AppItem key={each.id} details={each} />
              ))}
            </ul>
          </div>
        </div>
